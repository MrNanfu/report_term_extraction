...
word_probliang_or_e_major={"导管":0.01,"细胞":0.01,"腺体":0.01,"梭形细胞病变":0.01,"梭形细胞肿瘤":0.01,"导管内瘤":0.01, "肿瘤病变":0.01, "导管内乳头状病变":0.01, "纤维上皮性病变":0.01, "肿瘤性病变":0.01, "导管内乳头状肿瘤":0.01,"叶状肿瘤":0.01,"放射状瘢痕〈B3,不确定的潜在恶性病变）":0.01,"非典型导管增生":0.01,"非典型导管上皮增生":0.01, "导管上皮不典型增生":0.01, "导管上皮非典型增生":0.01, "平坦型上皮非典型增生":0.01,"平坦上皮非典型性":0.01,"导管上皮轻度非典型增生":0.01,"不典型增生":0.01,"导管上皮增生活跃":0.01, "非典型增生":0.01, "不典型导管增生":0.01, "导管上皮增生":0.01,"导管上皮增生稍活跃(B3,不确定的潜在恶性病变）":0.01,"上皮源性肿瘤":0.01,"BI-RADS 0": 0.01}
word_probliangxing_major={"良性":0.01,"普通型增生":0.01, "肉芽肿":0.01,"肉芽肿性炎":0.01, "硬化性腺病":0.01, "错构瘤":0.01, "导管内乳头状瘤":0.01, "导管内乳头状瘤炎性病变":0.01,"炎性":0.01,"炎症性改变":0.01,"炎症性":0.01,"炎症改变":0.01,"小叶性肉芽肿性乳腺炎":0.01,"小叶肉芽肿性乳腺炎":0.01,"肉芽肿性小叶炎":0.01,"肉芽肿性小叶性乳腺炎":0.01,"非特异性炎症性病变纤维上皮肿瘤":0.01, "非特异性炎症性病变":0.01, "纤维上皮性":0.01, "纤维上皮性肿瘤":0.01,"良性纤维上皮性肿瘤":0.01,"纤维腺瘤样增生":0.01,"纤维腺瘤":0.01,"纤维腺瘤改变":0.01,"幼年型纤维腺瘤":0.01,"良性叶状肿瘤":0.01,"泌乳性腺瘤":0.01,"管状腺瘤":0.01,"结节性泌乳性增生硬化性腺病":0.01, "结节性泌乳性增生":0.01, "错构瘤硬化性腺病":0.01, "复杂硬化性":0.01, "复杂性硬化性":0.01, "复杂硬化性病变":0.01, "表皮样囊肿":0.01, "复杂性硬化性病表皮样囊肿":0.01,"纤维囊性乳腺病":0.01,"纤维囊性变":0.01,"囊肿性病变":0.01, "积乳囊肿":0.01, "囊肿":0.01,"导管扩张症新辅助治疗后改变":0.01,"化疗后改变":0.01,"未见确切恶性肿瘤细胞":0.01,"未见肿瘤":0.01,"BI-RADS 1":0.01,"BI-RADS 2":0.01,"BI-RADS 3":0.01,'BI-RADSⅢ':0.01, "BI-RADS 4A":0.01,
...
def pathologicalfuc(pathological_bodypart, pathological_report):#给parse输入的四个参数里的前两个有关病理的参数赋值给该函数
    ...
    word_prob_negative_word_all={'排除':0.01,'鉴别':0.01,'不排除':0.01,'不完全除外':0.01,'不能除外':0.01,'不除外':0.01,'除外':0.01,'未见': 0.01}
    ...
    word_prob_negative_word2 = {'不排除':0.01,'不完全除外': 0.01, '不能除外': 0.01, '不除外': 0.01 ,'考虑': 0.01, '疑' : 0.01}
    ...
    if len(segmentsb_negative_word_all)!=0:
        ...
        if len(invalid_b)==0:
            ...
            '''
            4_raw（从语句中提取的良恶性也需要进行删减）
            '''
            segmentsb4_raw_new = []
            for i in range(int(len(segmentsb4_raw) / 2)):
                if segmentsb4_raw[2 * i + 1] < loc_comma or segmentsb4_raw[2 * i + 1] > loc_full_stop:
                    segmentsb4_raw_new.append(segmentsb4_raw[2 * i])
                    segmentsb4_raw_new.append(segmentsb4_raw[2 * i + 1])
            segmentsb4_raw = segmentsb4_raw_new
        ...
        def remove_liange(segmentsb4_raw, segmentsb_negative_word1):
            segmentsb4_after=[]
            num_cnt = int(len(segmentsb4_raw)/2)
            cnt = 0
            if int(len(segmentsb_negative_word1)) != 0:
                for i in range(int(len(segmentsb4_raw)/2)):
                    for j in range(int(len(segmentsb_negative_word1)/2)):
                        if segmentsb4_raw[2*i+1]-segmentsb_negative_word1[2*j+1] > 10 or segmentsb4_raw[2*i+1]-segmentsb_negative_word1[2*j+1] < -10:
